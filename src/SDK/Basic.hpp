...
public:
	...
	{
		// HUDFix: this game's FNamePool layout differs from the SDK default, so use the
		// game's FName::AppendString (offset captured by Dumper) instead of decoding GNames.
		// AppendString writes the full display name (incl. any numeric suffix) into Out.
		static void(*AppendStr)(const void*, void*) = nullptr;
		if (!AppendStr)
			AppendStr = reinterpret_cast<void(*)(const void*, void*)>(InSDKUtils::GetImageBase() + Offsets::AppendString);

		wchar_t buf[1024];
		struct FStrTmp { wchar_t* Data; int32_t Num; int32_t Max; } s{ buf, 0, 1024 };
		AppendStr(reinterpret_cast<const void*>(this), &s);

		int n = s.Num; if (n < 0) n = 0; if (n > 1023) n = 1023;
		std::string RetStr; RetStr.reserve(n);
		for (int i = 0; i < n; ++i) { wchar_t c = buf[i]; if (c == 0) break; RetStr += static_cast<char>(c); }
		return RetStr;
	}
